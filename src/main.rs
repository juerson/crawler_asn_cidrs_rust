...
fn get_user_input() -> String {
    loop {
        print!("输入一个ASN自治系统编号(比如：AS13335、AS16509、AS31898等):");
        io::stdout().flush().expect("刷新输出缓冲区失败");
...
fn select_url(asn: &str) -> String {
    println!("\n选择从哪个网站下载：");
    println!("【1】从bgp.he.net网站下载");
    println!("【2】从whoisrequest.com网站下载\n");
    loop {
        print!("输入上面对应的选项:");
        io::stdout().flush().expect("刷新输出缓冲区失败");
        ...
        match trimmed_input {
            "1" => {
                return format!("https://bgp.he.net/{}#_prefixes", asn);
            }
            "2" => {
                return format!("https://whoisrequest.com/ip/{}", asn);
            }
            _ => {
                // println!("无效选项，请重新输入。");
            }
...
fn process_document(document: &Document, domain: &str) -> Vec<String> {
    let mut matched_texts: Vec<String> = Vec::new();
    println!("");
    match domain {
        "bgp.he.net" => {
            for id in &["prefixes", "prefixes6"] {
                if let Some(prefix_div) = document.find(|n: &Node| {
                    n.name() == Some("div")
                        && n.attr("id").map(|attr| attr == *id).unwrap_or(false)
                }).next() {
                    for node in prefix_div.find(|n: &Node| n.name() == Some("a")) {
                        let text = node.text();
                        let trimmed_text = text.trim().to_string();
                        if IPV4_CIDR_RE.is_match(&trimmed_text) || IPV6_CIDR_RE.is_match(&trimmed_text) {
                            println!("{}", &trimmed_text);
                            matched_texts.push(trimmed_text);
                        }
                    }
                }
            }
        ...
        "whoisrequest.com" => {
            if let Some(prefix_table) = document.find(|n: &Node| {
                n.name() == Some("table") && n.attr("id").map(|attr| attr == "prefix-table").unwrap_or(false)
            }).next() {
        ...
        _ => {
            // Handle other domains or provide a default behavior
        }
    }
...
async fn get_asn_data(asn: &str) -> Result<(), Box<dyn std::error::Error>> {
    // 调用一个函数，选择从哪个网站下载（选择哪个url）
    let url = select_url(&asn);
    ...
    if response.status().is_success() {
        ...
        // 解析url链接
        let parsed_url = Url::parse(&url).expect("URL解析失败");

        // 获取域名
        let domain = parsed_url.domain().expect("无法获取域名");
        
        // 处理网页的数据（清洗数据）
        let matched_texts = process_document(&document,domain);
        ...
        match matched_texts.is_empty() {
            ...
            false => {
                let mut file = File::create(format!("{}_{}_CIDRs.txt", asn, domain))?;
                // 遍历最终获取的结果，并写入文件中
                for text in matched_texts {
                    writeln!(file, "{}", text)?;
                    file.flush()?; // 立即刷新文件缓冲,将数据刷新到文件
                }
                println!("\n{}自治系统中，公开的IPv4、IPv6前缀(也就是CIDR)，已经下载成功！耗时：{:?}", asn, start.elapsed());
            }
        }
    } else {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("本工具：用于下载网站bgp.he.net或whoisrequest.com中，公开的IPv4、IPv6前缀(也就是CIDR)。\n");
