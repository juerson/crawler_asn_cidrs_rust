...
async fn get_asn_data(asn: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if response.status().is_success() {
        ...
        for id in &["prefixes", "prefixes6"] {
            // 找到具体的 <div id='prefixes'> 或 <div id='prefixes6'> 元素（在这两个标签中找a标签，排除这两个标签外的其他a标签）
        ...
        match matched_texts.is_empty() {
            ...
            false => {
                let mut file = File::create(format!("{}_bgp.he.net_CIDRs.txt", asn))?;
                // 打印匹配成功的文本（最终获取的结果）
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("本工具：用于下载网站 https://bgp.he.net/ 中，公开的IPv4、IPv6前缀(也就是CIDR)。\n");
